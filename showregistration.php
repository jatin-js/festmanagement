...
include('db.php');
$regid = $_GET["regid"];
$query = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM registration WHERE id='$regid'"));
$eventquery = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM events WHERE name='$query[6]'"));
$coordinatorquery = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM coordinators WHERE event_name='$query[6]'"));
$status = $query[8];
$eventstatus = $eventquery[13];
$amount;
if ($query[3] == 'NIT Andhra') {
    $amount = $eventquery[5];
} else {
    $amount = $eventquery[6];
}
...
<body>
    ...
    <div class="container">
        ...
        <div style="margin:0 auto;width:1100px;color:black;overflow:scroll" id="block">
