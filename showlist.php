...
if (!isset($_POST['college']) && !isset($_POST['event'])) {
	...
	if ($mode == 0)
		$query = mysqli_query($link, "SELECT * FROM registration");
	elseif ($mode == 1)
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=1");
	else
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=0");
} elseif (isset($_POST['college'])) {
	$clg = $_POST['college'];
	if ($mode == 0)
		$query = mysqli_query($link, "SELECT * FROM registration WHERE collegename='$clg'");
	elseif ($mode == 1)
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=1 AND collegename='$clg'");
	else
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=0 AND collegename='$clg'");
} elseif (isset($_POST['event'])) {
	$event = $_POST['event'];
	if ($mode == 0)
		$query = mysqli_query($link, "SELECT * FROM registration WHERE eventname='$event'");
	elseif ($mode == 1)
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=1 AND eventname='$event'");
	else
		$query = mysqli_query($link, "SELECT * FROM registration WHERE paid=0 AND eventname='$event'");
}
?>
<html lang="en">
