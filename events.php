...
include('db.php');
$query = mysqli_query($link, "SELECT * FROM events WHERE id<>1");
?>
<html lang="en">
				...
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($query)) {
							$totalreg = mysqli_fetch_array(mysqli_query($link, "SELECT count(id) FROM `registration` WHERE eventname='$row[1]'"));
							$paidreg = mysqli_fetch_array(mysqli_query($link, "SELECT count(id) FROM `registration` WHERE eventname='$row[1]' AND paid=1"));
							echo "<tr>";
